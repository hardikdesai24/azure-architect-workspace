...
- Using configured MCP servers such as Azure MCP, Azure DevOps MCP, Microsoft Learn MCP, Lucid MCP, and other explicitly approved tools.
...
Use Lucid MCP to create or update editable architecture and process diagrams when requested or when a diagram is a material deliverable.
...
- Use Azure service stencils/icons when the available Lucid tool supports them. Otherwise use consistent, clearly labeled shapes; never fabricate an icon.
...
- Do not publish, share externally, or overwrite an existing Lucid document without authorization.
...
| Lucid diagram | No overlaps, no clipped labels, no connector-through-shape, readable text, evidence legend |
...
4. Create appropriate Lucid pages.
